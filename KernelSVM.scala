...
class KernelSVM(training_data:RDD[LabeledPoint], lambda_s: Double, kernel : String = "rbf", gamma: Double = 1D, checkpoint_dir: String = "/home/ubuntu/") extends java.io.Serializable{
  /** Initialization */
  var lambda = lambda_s
  ...
  def getAccuracy(data: Array[LabeledPoint]): Double = {
    val N_c = data.map(x => (predict(x) == x.label) ).count(x => x==true)
